...
from models.embedders import BERTEncoder, OneHotClassEmbedding, UnconditionalClassEmbedding, GloveEmbedding
import torch
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, (imgs, labels, captions) in enumerate(train_loader):
            ...
            if (i + 1) % opt.print_every == 0:
                ...
                if(loss_to_log < 3.95):
                    sample_image(model, embedder, opt.output_dir, n_row=4,batches_done=batches_done,dataloader=val_loader,device=device)
                loss_to_log = 0.0
                torch.save(model.state_dict(),
                   os.path.join(opt.output_dir, 'models', '2batch_{}.pt'.format(i)))
...
def eval(model, embedder, test_loader):
    ...
    model = model.eval()
    sample_image(model, embedder, 'outputs/pixelcnn', n_row=4,batches_done=1,dataloader=test_loader,device=torch.device('cuda'))
    # bpd = 0.0
